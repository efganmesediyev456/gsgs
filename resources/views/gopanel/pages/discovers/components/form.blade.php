<form id="saveForm" action="{{$route}}" enctype="multipart/form-data">
    ...
    @foreach($languages as $lang)
        <div  @class(['tab-pane','fade', 'show'=>$loop->index==0, 'active'=>$loop->index==0, 'mt-4']) id="{{$lang->locale}}" role="tabpanel" aria-labelledby="{{$lang->locale}}-tab">
            ...
            <div class="mb-3">
                <label for="category-name" class="form-label">Başlıq2 ({{$lang->locale}})</label>
                <input type="text" class="form-control" id="category-name" name="title2[{{$lang->locale}}]" value="{{$item->getTranslation('title2', $lang->locale)}}" placeholder="Başlıq2 daxil edin">
            </div>
            <div class="mb-3">
                <label for="category-name" class="form-label">Başlıq3 ({{$lang->locale}})</label>
                <input type="text" class="form-control" id="category-name" name="title3[{{$lang->locale}}]" value="{{$item->getTranslation('title3', $lang->locale)}}" placeholder="Başlıq3 daxil edin">
            </div>
        </div>
    @endforeach
    <div class="mb-3">
