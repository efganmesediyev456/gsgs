...
class DiscoverController extends GopanelController
{
    ...
    {
        return view('gopanel.pages.discovers.index');
    }

    public function updateOrSave(Discover $item)
    {
//        $this->checkPermission($item,'features.update','features.store');
        ...
        try {
            if (is_null($item))
                $item = new Discover;
            $route = route("gopanel.discovers.save", $item);
            $this->response['html'] = View::make('gopanel.pages.discovers.components.form', [
                'item'          => $item,
                'route'         => $route
            ])->render();
    ...
    public function save(Discover $item, Request $request)
    {
//        $this->checkPermission($item,'features.update','features.store');
        ...
        try {
            ...
            if ($request->hasFile('image')){
                $data['image'] = FileUploadHelper::uploadFile($request->file('image'), "discovers", $data['image']);
            }
